...
local startRebirths = player:WaitForChild("leaderstats"):WaitForChild("Rebirths").Value
...
local function parseInput(text)
    ...
    elseif result:find("t") then multiplier = 1000000000000 end
    local num = tonumber(result:match("[%d%.]+"))
    return num and (num * multiplier) or 0
end

WindUI.Services.CustomTextKey = {
    Name = "Key System",
    Icon = "key", 
    Args = { "CorrectKey" }, 
        ...
        return {
            Verify = function(userInput)
                if userInput == CorrectKey then
                    ...
                    return false, "Key Salah! Silakan coba lagi."
                end
            end,
            Copy = function()
                setclipboard("ZorVex") 
...
local Window = WindUI:CreateWindow({
    ...
    User = {
        ...
        Callback = function()
            ...
            WindUI:SetTheme(themes[currentThemeIndex])
            WindUI:Notify({
...
ImgBtn.MouseButton1Click:Connect(function()
    local shrink = TweenService:Create(ImgBtn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 70, 0, 70)})
    local grow = TweenService:Create(ImgBtn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 90, 0, 90)})
    shrink:Play()
...
farmPlusTab:Input({
    ...
    Locked = true,
        LockedTitle = "VIP Only Contact ZorVex Now",
    })
